...
void AddCircumcircleA(const bool& circleA, const double & BExternalGlobaltemp) {//be careful that A_x_mat will change whenever calcA() is called;
	if (circleA==true){
	    using namespace arma;
	    using namespace VECTORPOT;
		//save A from only effective magnetic field on the rectangle
	    mat gx = A_x_mat;
	    mat gy = A_y_mat;
		//calculate A from only real uniform magnetic field on the rectangle and save to z-xy
	    mat zd(Nx, Ny);
	    ...
	    mat zx = A_x_mat;
	    mat zy = A_y_mat;
		//analytic solution of A from real uniform magnetic field on the circle (including the rectangle) and save to b-xy
    	mat bx(Nx, Ny);
	    mat by(Nx, Ny);
	    bx.fill(0.0);
	    by.fill(0.0);
		...
		//sum up rectange A: gx and the circumcircle A: bx - zx
		bx = (bx - zx) * (1.0 + BExternalGlobaltemp);
		by = (by - zy) * (1.0 + BExternalGlobaltemp);
		A_x_mat = gx + bx;
		A_y_mat = gy + by;
	}
}
